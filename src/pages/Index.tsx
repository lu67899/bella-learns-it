...
import { BookOpen, BrainCircuit, CalendarDays, StickyNote, Sparkles, GraduationCap, MessageCircle, Send, X, ChevronRight } from "lucide-react";
...
const Index = () => {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      const [rRes, fRes, cRes, modRes, topRes, mRes] = await Promise.all([
        ...
        supabase.from("modulo_topicos").select("modulo_id"),
      ...
      // Count topicos per modulo
      const topicoCount = new Map<string, number>();
      (topRes.data || []).forEach((t: any) => {
        topicoCount.set(t.modulo_id, (topicoCount.get(t.modulo_id) || 0) + 1);
      });
      const mods = (modRes.data || []).map((m: any) => ({
        ...m,
        topicos_count: topicoCount.get(m.id) || 0,
      }));
      setModulos(mods);
